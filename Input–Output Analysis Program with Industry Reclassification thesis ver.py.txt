...
if choice == "0. 한국은행 데이터 정합성 검증":
    ...
    **논문 검증용:** 한국은행의 기본 33개 대분류 체계가 165개 소분류와 어떻게 매핑되어 통합행렬을 구성하는지 확인하고, 
    ...
    # 코드가 알아서 165개 소분류가 어디에 속하는지 찾아서 정리해주는 마법의 로직
    mapping_data = []
    for i, major_name in enumerate(base_sector_names):
        # 해당 대분류(i)에 1로 체크된 소분류들을 싹 찾아냄
        sub_indices = np.where(pmb_m[i, :] > 0)[0] 
        sub_names = [industry_list[idx] for idx in sub_indices]
        mapping_data.append({
            "대분류 (33개)": major_name, 
            "포함된 소분류 개수": f"{len(sub_names)}개", 
            "포함된 소분류 목록 (자동 체크됨)": ", ".join(sub_names)
        })
    ...
    if st.button("한국은행 기본 33개 부문 분석 실행"):
        with st.spinner('검증용 산업연관분석 계산 중...'):
            # pm을 기본 pmb_m(33x165)으로 그대로 씁니다 (신설 산업 없음)
            pm_base = pmb_m
            n = pm_base.shape[0]
            ...
            st.success("기본 분석 완료! 아래 표의 값들을 한국은행 엑셀 데이터와 비교하여 논문에 정합성을 제시하세요.")
            
        # 결과 보여주기
        if "Lf_base" in st.session_state:
            tab_base1, tab_base2 = st.tabs(["생산 유발계수 (Lf)", "부가가치 유발계수 (Lv)"])
            with tab_base1:
                st.dataframe(pd.DataFrame(st.session_state["Lf_base"], index=base_sector_names, columns=base_sector_names).style.background_gradient(cmap="Blues"))
            with tab_base2:
                st.dataframe(pd.DataFrame(st.session_state["Lv_base"], index=base_sector_names, columns=base_sector_names).style.background_gradient(cmap="Greens"))
