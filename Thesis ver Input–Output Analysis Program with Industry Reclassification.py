...
menu = ["0. 한국은행 데이터 정합성 검증", "1. 산업 재분류 설정", "2. 통합행렬(pm) 생성", "3. 산업연관분석 실행", "4. 분석 결과 및 시각화"]
choice = st.sidebar.radio("단계 선택", menu)

# 0. 한국은행 데이터 정합성 검증 탭
if choice == "0. 한국은행 데이터 정합성 검증":
    st.title("Step 0. 한국은행 데이터 정합성 검증")
    st.markdown("""
    **논문 검증용:** 한국은행의 기본 33개 대분류 체계가 165개 소분류와 어떻게 매핑되어 통합행렬을 구성하는지 확인하고, 
    조작 없는 기본 상태에서의 산업연관분석 결과를 산출하여 한국은행 공식 발표 자료와 대조합니다.
    """)

    if pmb_m is not None and base_sector_names is not None:
        # 1. 대분류-소분류 매핑 과정 시각화
        st.subheader("1. 대분류별 소분류 포함(체크) 현황")
        st.caption("기존 행통합행렬 데이터에 기반하여, 각 대분류에 어떤 소분류들이 통합되는지 자동으로 정리된 표입니다.")
        
        mapping_data = []
        for i, major_name in enumerate(base_sector_names[:33]):
            sub_indices = np.where(pmb_m[i, :] > 0)[0] 
            sub_names = [industry_list[idx] for idx in sub_indices]
            mapping_data.append({
                "대분류명": major_name, 
                "통합된 소분류 개수": f"{len(sub_names)}개", 
                "포함된 소분류 목록": ", ".join(sub_names)
            })
        ...
        # 2. 분석 실행 및 결과 도출
        st.markdown("---")
        st.subheader("2. 기본 통합행렬 기반 분석 실행")
        st.write("위의 매핑 구조를 바탕으로 33x33 기본 통합행렬을 생성하고 유발계수를 계산합니다.")
        
        if st.button("한국은행 기본 33개 부문 분석 실행"):
            with st.spinner('검증용 산업연관분석 계산 중...'):
                try:
                    # 조작 없는 원본 통합행렬(33x165)
                    pm_base = pmb_m
                    n = pm_base.shape[0]

                    # (1) 생산유발계수(Lf_base) 계산
                    Ct = Tts_m[-1, 0:165] @ pm_base.T
                    Z = pm_base @ Ttsd_m[:165, :165] @ pm_base.T
                    A = Z / np.tile(Ct, reps=[n,1])
                    I = np.eye(n)
                    Lf_base = np.linalg.inv(I - A)

                    # (2) 부가가치유발계수(Lv_base) 계산
                    Va = Tts_m[-2, :165] @ pm_base.T
                    V_ratio = Va / Ct
                    V_m = np.diagflat(V_ratio)
                    Lv_base = V_m @ Lf_base

                    # (3) 취업유발계수(Le_base) 계산 - 선생님 기존 코드 원형 복원
                    Ee = (Ttse_m[0:165, 0].reshape(1, 165).astype(float) @ pm_base.T)
                    Ee_ratio = ((Ee * 1000 / Ct.reshape(1, n))).astype(float)
                    Ee_m = np.diagflat(Ee_ratio)
                    Le_base = Ee_m @ Lf_base

                    # (4) 고용유발계수(Lw_base) 계산 - 선생님 기존 코드 원형 복원
                    Ew = (Ttse_m[0:165, 1].reshape(1, 165).astype(float) @ pm_base.T)
                    Ew_ratio = ((Ew * 1000 / Ct.reshape(1, n))).astype(float)
                    Ew_m = np.diagflat(Ew_ratio)
                    Lw_base = Ew_m @ Lf_base

                    # 세션에 결과 저장
                    st.session_state["Lf_base"] = Lf_base
                    st.session_state["Lv_base"] = Lv_base
                    st.session_state["Le_base"] = Le_base
                    st.session_state["Lw_base"] = Lw_base

                    st.success("기본 분석 완료! 아래 탭에서 한국은행 공식 발표 값과 직접 대조해보세요.")
                except Exception as e:
                    st.error(f"계산 중 에러가 발생했습니다: {e}")

        # 3. 탭 분리하여 결과 출력 (취업, 고용 분리)
        if "Lf_base" in st.session_state:
            tab1, tab2, tab3, tab4 = st.tabs(["생산 유발계수 ($L_f$)", "부가가치 유발계수 ($L_v$)", "취업 유발계수 ($L_e$)", "고용 유발계수 ($L_w$)"])
            cols = base_sector_names[:33]
            
            with tab1:
                st.subheader("생산 유발계수 행렬")
                st.dataframe(pd.DataFrame(st.session_state["Lf_base"], index=cols, columns=cols).style.background_gradient(cmap="Blues"))
            
            with tab2:
                st.subheader("부가가치 유발계수 행렬")
                st.dataframe(pd.DataFrame(st.session_state["Lv_base"], index=cols, columns=cols).style.background_gradient(cmap="Greens"))

            with tab3:
                st.subheader("취업 유발계수 행렬")
                st.dataframe(pd.DataFrame(st.session_state["Le_base"], index=cols, columns=cols).style.background_gradient(cmap="Oranges"))
                
            with tab4:
                st.subheader("고용 유발계수 행렬")
                st.dataframe(pd.DataFrame(st.session_state["Lw_base"], index=cols, columns=cols).style.background_gradient(cmap="Oranges"))
