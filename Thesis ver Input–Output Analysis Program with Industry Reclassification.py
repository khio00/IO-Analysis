...
# 0. 한국은행 데이터 정합성 검증 탭 (에러 수정 및 아이디어 반영 완벽 통합)
# ==========================================
if choice == "0. 한국은행 데이터 정합성 검증":
    ...
    **논문 검증용:** 한국은행의 기본 33개 대분류 체계가 165개 소분류와 어떻게 매핑되어 통합행렬을 구성하는지 확인하고, 
    ...
    if pmb_m is not None and base_sector_names is not None:
        ...
        # 2. 분석 실행 및 결과 도출 (선생님의 3개 탭 분리 아이디어 적용)
        st.markdown("---")
        ...
        # 분석 결과가 세션에 저장되어 있으면 3개의 탭으로 나누어 출력
        if "Lf_base" in st.session_state:
            tab1, tab2, tab3 = st.tabs(["생산 유발계수 ($L_f$)", "부가가치 유발계수 ($L_v$)", "고용 유발계수 ($L_w$)"])
        ...
        st.error("데이터 로드에 실패하여 검증을 진행할 수 없습니다. 터미널의 오류 메시지를 확인하세요.")
