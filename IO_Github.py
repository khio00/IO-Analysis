    ...
    if "Lf" not in st.session_state:
        ...
        def show_detail_tab(effect_name, matrix, direct_vec=None, is_production=False):
                ...
                with c_col:
                    ...
                    st.dataframe(top10_col.reset_index(drop=True).style.bar(subset=["유발액"], color="#d65f5f"))
                ...
                with c_row:
                    ...
                    st.dataframe(top10_row.reset_index(drop=True).style.bar(subset=["유발액"], color="#4682B4"))
